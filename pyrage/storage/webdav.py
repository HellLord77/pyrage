...
class WebDAVStorage(Storage):
    def __init__(self, hostname: str, login: str | None = None, password: str | None = None):
        self._client = Client(
            {
        ...
        self._client.verify = urlparse(hostname).scheme == "https"
        super().__init__()
    ...
    def _get_file(self, file: File) -> Readable:
        return ReadableResponse(
            self._client.execute_request(action="download", path=Urn(self._client.root + file.path).quote()),
        )
